...
'''
    ...
    Funcion:    Parcha las operaciones de TensorFlow incompatibles
                de un modelo de CNN para ser cargado en TensorRT (SSD original)
    Entradas:   Path del archivo frozen graph de la red SSD original (.pb) TF1
    Salidas:    Modelo de la CNN parchada de tipo de archivo gs.DynamicGraph
'''
def fix_tf_ssd_model1(path):
...
def convert_to_tensorrt(path_model):
    dynamic_graph = fix_tf_ssd_model1(path_model)
    uff_model = uff.from_tensorflow(dynamic_graph.as_graph_def(), output_name, output_filename='ssd_model.uff')
    TRTbin = "TRT_ssd_model.bin"
    ...
    with trt.Builder(TRT_LOGGER) as builder, builder.create_network() as network, trt.UffParser() as parser:
        ...
        parser.parse('ssd_model.uff', network)
...
if __name__ == "__main__":
    if(len(sys.argv) != 2):
        print("Forma de uso: python3 ConfigSSD1.py path_frozen_graph.pb")
        exit()
